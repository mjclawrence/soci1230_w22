...
Note the additional options added to the first line of the code chunk above. They suppress any warnings, errors, and messages accompanying whatever code is run. (Loading the tidyverse packages creates a lot of output, for example.) You'll still that output in this noteboook but it won't be in the knitted file.
...
```{r}
(act05_plot_dodge + ylim(c(0,.7))) / (goal07_plot_dodge + ylim(c(0,.7)))
```
...
```{r}
((goal07_plot_dodge + ylim(c(0,.5))) + act05_plot_stack_multiple) +
  plot_annotation(
...
weighted_mean_summary |> 
  kbl(booktabs = TRUE, # Adds default styling
      ...
      col.names = c("Institution Type", "50 miles or less", "51 to 100 miles",
                    "More than 100 miles"),
      caption = "Proportion Of Students Traveling Each Distance From Home To Campus, By College Type") |>  # This becomes the title; use it to automatically number tables
  kable_styling("hover") # works with html for html output
  # kable_styling(latex_options = "striped") # add latex_options for pdf output
```
