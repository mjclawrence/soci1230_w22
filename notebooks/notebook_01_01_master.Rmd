---
title: "SOCI 1230, Week One, Class One"
...
In this morning's session, we installed the `dplyr` package and saw how to load it with the `library()` function. That package is part of a suite of packages called the `tidyverse`. Let's install all the associated packages and load them. I put a hashtag in front of the first line so it would not run when we knit the file. Remove the hashtag before running this chunk.
...
```{r}
mobility$kq5_cond_parq1[mobility$name=="Middlebury College"]
```
...
```{r}
mobility$kq5_cond_parq5[mobility$name=="Middlebury College"]
```
