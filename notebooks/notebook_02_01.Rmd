...
Load the packages you might have used to complete Assignment OneLoad the data and tidyverse

```{r}
library(tidyverse)
profiles <- read_csv("") # use your path here
```
...
Let's make two new plots for this example. We'll compare the responses to the `GOAL07` question in the TFS and the CSS.

Here's the CSS...
```{r}
goal07_css_plot_dodge <- css_summary |>  
  filter(Name == "GOAL07") |> 
  mutate(Label = factor(Label, # use the Label variable here
...
And here's the TFS...
```{r}
goal07_tfs_plot_dodge <- tfs_summary |>  
  filter(Name == "GOAL07") |> 
  mutate(Label = factor(Label, # use the Label variable here
                          levels = c("Not important",
                                     "Somewhat important",
...
```{r}
goal07_tfs_plot_dodge + goal07_css_plot_dodge
```
...
```{r}
act05_plot_stack | (goal07_tfs_plot_dodge / goal07_css_plot_dodge)
```
