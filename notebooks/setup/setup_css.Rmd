...
css_means <- college_mobility.filtered |> 
  ...
  summarise(
            ...
            across(starts_with("NATENG"),
                   list(no_maen = ~mean(.x==1, na.rm = TRUE),
                        yes_mean = ~mean(.x==2, na.rm = TRUE), 
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
  ...
  summarise_at(vars(-n_responses), 
                   list(cor_n_colleges = ~sum(!is.na(.x)),
                        ...
                        cor_par_q5 = ~wtd.cor(.x, par_q5, 
                                              weight = n_responses)[1]),
                   .names = "{.col}_{.fn}") |> 
  pivot_longer(names_to = "Variable", values_to = "Correlation", 1:42576) |>
...
```{r}
write.csv(css_correlations, "../../data/css_correlations_withno.csv", row.names = FALSE)
```
