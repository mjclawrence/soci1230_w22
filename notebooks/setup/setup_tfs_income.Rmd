...
```{r}
table(tfs_subset_master$INCOME)

tfs_subset_master <- tfs_subset_master |> 
  mutate(income_cat = ifelse(INCOME<=12, 0, 1)) # 3, 5, 7, 10, 11 do not exist; lowest seven are 1-12.

tfs_subset_master |> count(income_cat)

income_cat_summary <- tfs_subset_master |> 
  group_by(campus_id) |> 
...
```{r}
tfs_means_income <- tfs_subset_master |> 
...
```{r}
varnames_original <- gsub("(.+?)(\\..*)", "\\1", names(tfs_means_income))
varnames_summarize <- names(tfs_means_income)
...
```{r}
tfs_means_income_long <- tfs_means_income |> 
  mutate(income_cat = factor(income_cat,
                             levels = c(0,1),
                             labels = c("income_cat_0", "income_cat_1"))) |> 
  pivot_longer(names_to = "variable", values_to = "mean",
               4:ncol(tfs_means_income)) |> 
  select(-n_responses) |> 
  pivot_wider(names_from = income_cat, values_from = mean)

tfs_means_income_merge <- left_join(tfs_means_income_long, income_cat_summary)

```


```{r}
tfs_means_income_wide <- tfs_means_income_merge |> 
  pivot_longer(names_to = "income_category", values_to = "mean",
               starts_with("income_cat")) |> 
  pivot_wider(names_from = "variable", values_from = "mean")

write.csv(tfs_means_income_wide, "../../data/tfs_means_income.csv", row.names = FALSE)
```

```{r}
tfs_means_income_merge_wtd <- tfs_means_income_wide |> 
  pivot_longer(names_to = "variable", values_to = "mean",
               5:ncol(tfs_means_income_wide)) |> 
  #select(-n_responses) |> 
