...
```{r}
write.csv(tfs_means_mobility, "../../data/tfs_means_mobility_withno.csv", row.names = FALSE)
```
  ...
  summarise_at(vars(-n_responses), 
                   list(cor_n_colleges = ~sum(!is.na(.x)),
                        ...
                        cor_par_q5 = ~wtd.cor(.x, par_q5, 
                                              weight = n_responses)[1]),
                   .names = "{.col}_{.fn}") |> 
  pivot_longer(names_to = "Variable", values_to = "Correlation", 1:18672) |>
...
```{r}
write.csv(tfs_correlations, "../../data/tfs_correlations_withno.csv", row.names = FALSE)
```
