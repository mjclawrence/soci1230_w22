...
tfs_means <- tfs_subset_master |> 
  ...
  summarise(across(starts_with("CHOICE"),
            ...
            across(starts_with("DISTHOME"),
                   list(miles10less = ~mean(.x==1, na.rm = TRUE),
                        miles11to50 = ~mean(.x==2, na.rm = TRUE),
                        miles51to100 = ~mean(.x==3, na.rm = TRUE),
                        miles101to500 = ~mean(.x==4, na.rm = TRUE),
                        miles501plus = ~mean(.x==5, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            ...
            across(starts_with("INCOME"),
                   list(inclessthan50k = ~mean(.x<18, na.rm = TRUE),
            ...
            across(starts_with("PLANLIVE"),
                   list(family_mean = ~mean(.x==1, na.rm = TRUE),
                        ...
                        greekhouse_maen = ~mean(.x==4, na.rm = TRUE),
            ...
            across(starts_with("RACE01"),
                   list(amindian_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            across(starts_with("RACE10"),
                   list(puerto_rican_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            across(starts_with("RACE11"),
                   list(other_latino_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            across(starts_with("RACE12"),
                   list(white_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            across(starts_with("RACE13"),
                   list(other_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                    .names = "{.col}.{.fn}"),
            across(starts_with("RACE7102"),
                   list(asian_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
            across(starts_with("RACE8"),
                   list(black_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
             across(starts_with("RACE9"),
                   list(mexican_chicano_mean = ~mean(.x==2, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.{.fn}"),
...
```{r}
write.csv(tfs_means_mobility, "../../data/tfs_means_mobility.csv", row.names = FALSE)
```
...
```{r}
tfs_correlations <- tfs_means_mobility |> 
  select(-c(campus_id, iclevel, type, tier_name, state, region, ends_with("propna"))) |>
  summarise_at(vars(-n_responses), 
                   list(cor_n_colleges = ~sum(!is.na(.x)),
                        ...
                        cor_k_rank = ~wtd.cor(.x, k_rank, 
                                              weight = n_responses)[1],
                        cor_k_q1 = ~wtd.cor(.x, k_q1, 
                                            weight = n_responses)[1],
                        cor_k_q2 = ~wtd.cor(.x, k_q2, 
                                            weight = n_responses)[1],
                        cor_k_q3 = ~wtd.cor(.x, k_q3, 
                                            weight = n_responses)[1],
                        cor_k_q4 = ~wtd.cor(.x, k_q4, 
                                            weight = n_responses)[1],
                        ...
                        cor_k_rank_cond_parq5 = ~wtd.cor(.x, k_rank_cond_parq5, 
                                                         weight = n_responses)[1],
                        cor_kq1_cond_parq1 = ~wtd.cor(.x, kq1_cond_parq1, 
                                                      weight = n_responses)[1],
                        cor_kq2_cond_parq1 = ~wtd.cor(.x, kq2_cond_parq1, 
                                                      weight = n_responses)[1],
                        cor_kq3_cond_parq1 = ~wtd.cor(.x, kq3_cond_parq1, 
                                                      weight = n_responses)[1],
                        cor_kq4_cond_parq1 = ~wtd.cor(.x, kq4_cond_parq1, 
                                                      weight = n_responses)[1],
                        cor_kq5_cond_parq1 = ~wtd.cor(.x, kq5_cond_parq1, 
                                                      weight = n_responses)[1],
                        cor_kq1_cond_parq2 = ~wtd.cor(.x, kq1_cond_parq2, 
                                                      weight = n_responses)[1],
                        cor_kq2_cond_parq2 = ~wtd.cor(.x, kq1_cond_parq2, 
                                                     weight = n_responses)[1],
                        cor_kq3_cond_parq2 = ~wtd.cor(.x, kq1_cond_parq2, 
                                                      weight = n_responses)[1],
                        cor_kq4_cond_parq2 = ~wtd.cor(.x, kq1_cond_parq2, 
                                                      weight = n_responses)[1],
                        cor_kq5_cond_parq2 = ~wtd.cor(.x, kq1_cond_parq2, 
                                                      weight = n_responses)[1],
                        cor_kq1_cond_parq3 = ~wtd.cor(.x, kq1_cond_parq3, 
                                                      weight = n_responses)[1],
                        cor_kq2_cond_parq3 = ~wtd.cor(.x, kq2_cond_parq3, 
                                                      weight = n_responses)[1],
                        cor_kq3_cond_parq3 = ~wtd.cor(.x, kq3_cond_parq3, 
                                                      weight = n_responses)[1],
                        cor_kq4_cond_parq3 = ~wtd.cor(.x, kq4_cond_parq3, 
                                                      weight = n_responses)[1],
                        cor_kq5_cond_parq3 = ~wtd.cor(.x, kq5_cond_parq3, 
                                                      weight = n_responses)[1],
                        cor_kq1_cond_parq4 = ~wtd.cor(.x, kq1_cond_parq4, 
                                                      weight = n_responses)[1],
                        cor_kq2_cond_parq4 = ~wtd.cor(.x, kq2_cond_parq4, 
                                                      weight = n_responses)[1],
                        cor_kq3_cond_parq4 = ~wtd.cor(.x, kq3_cond_parq4, 
                                                      weight = n_responses)[1],
                        cor_kq4_cond_parq4 = ~wtd.cor(.x, kq4_cond_parq4, 
                                                      weight = n_responses)[1],
                        cor_kq5_cond_parq4 = ~wtd.cor(.x, kq5_cond_parq4, 
                                                      weight = n_responses)[1],
                        cor_kq1_cond_parq5 = ~wtd.cor(.x, kq1_cond_parq5, 
                                                      weight = n_responses)[1],
                        cor_kq2_cond_parq5 = ~wtd.cor(.x, kq2_cond_parq5, 
                                                      weight = n_responses)[1],
                        cor_kq3_cond_parq5 = ~wtd.cor(.x, kq3_cond_parq5, 
                                                      weight = n_responses)[1],
                        cor_kq4_cond_parq5 = ~wtd.cor(.x, kq4_cond_parq5, 
                                                      weight = n_responses)[1],
                        cor_kq5_cond_parq5 = ~wtd.cor(.x, kq5_cond_parq5, 
                                                      weight = n_responses)[1],
                        cor_par_q1 = ~wtd.cor(.x, par_q1, 
                                              weight = n_responses)[1],
                        cor_par_q2 = ~wtd.cor(.x, par_q2, 
                                              weight = n_responses)[1],
                        cor_par_q3 = ~wtd.cor(.x, par_q3, 
                                              weight = n_responses)[1],
                        cor_par_q4 = ~wtd.cor(.x, par_q4, 
                                              weight = n_responses)[1],
                        cor_par_q5 = ~wtd.cor(.x, par_q5, 
                                              weight = n_responses)[1]),
                   .names = "{.col}_{.fn}") |> 
  pivot_longer(names_to = "Variable", values_to = "Correlation", 1:18288) |>
...
tfs_correlations <- left_join(tfs_correlations, varnames) |> 
...
```{r}
write.csv(tfs_correlations, "../../data/tfs_correlations.csv", row.names = FALSE)
```
