...
goaltest <- college_mobility |>
  ...
  select(campus_id, starts_with("GOAL07"), k_rank)
...
prop.table(table(goaltest$GOAL07_TFS,
                 goaltest$GOAL07, exclude = NULL),1)

library(ggthemes)

goaltest |> 
  filter(!is.na(GOAL07_TFS), !is.na(GOAL07)) |> 
  group_by(GOAL07_TFS) |> 
  count(GOAL07) |> 
  na.omit() |> 
  mutate(prop = n/sum(n)) |> 
  ungroup() |> 
  ggplot(aes(x = GOAL07_TFS, y = n, fill = factor(GOAL07))) +
  geom_col() +
  theme_tufte() +
...
```
  ...
  ggplot(x = GOAL07_TFS, fill = GOAL07) + geom_bar()
```
