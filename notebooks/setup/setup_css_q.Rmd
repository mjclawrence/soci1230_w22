...
```{r}
chetty_fouryr <- read_csv("../../data/chetty_fouryr.csv")
...
```{r}
chetty_quantiles <- chetty_fouryr |> 
...
college_mobility_q.filtered <- left_join(college_mobility.filtered, chetty_vars)
```

```{r}
college_mobility_q.filtered <- college_mobility_q.filtered |> 
  pivot_longer(names_to = "q_var", values_to = "q_cat", starts_with("q_")) 
```


```{r}
css_means_q <- college_mobility_q.filtered |> 
  group_by(q_var, q_cat) |>
  summarise(
