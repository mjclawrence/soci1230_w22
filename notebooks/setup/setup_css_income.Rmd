...
```{r}
year_income <- college_mobility |> 
  count(YEAR, INCOME_TFS)
```
...
college_mobility.filtered <- college_mobility |> 
  select(all_of(all_filter_cols), INCOME_TFS)
```
...
There should be 14 income categories from the TFS questionnaire. The categories look messy and some of them don't seem to exist in the TFS data. Make those missing values. Then take the first seven categories (corresponding to up to 40k) and make them low income.

```{r}
missing_income_tfs_values <- c(3, 10, 11, 21, 24, 25)

college_mobility.filtered <- college_mobility.filtered |> 
  mutate(income_cat = ifelse(INCOME_TFS %in% missing_income_tfs_values, NA, INCOME_TFS),
         income_cat = ifelse(INCOME_TFS<=9, 0, 1)) # 3 is missing and 8 does not exist; lowest seven are 1-9.

college_mobility.filtered |> count(income_cat)

income_cat_summary <- college_mobility.filtered |> 
  group_by(campus_id) |> 
  summarise(n_responses = mean(n),
            mean_low_income = mean(income_cat==0, na.rm = TRUE))

no_all_low_income <- income_cat_summary |> 
  filter(mean_low_income == 0 | mean_low_income == 1 ) |> 
  pull(campus_id)

summary(income_cat_summary$mean_low_income)

income_cat_summary |> 
  filter(mean_low_income > 0) |> 
  ggplot(aes(x = mean_low_income)) + geom_density()
```

## Calculate means of all variables by college and by income cat

```{r}
css_means_income <- college_mobility.filtered |> 
  filter(campus_id %nin% no_all_low_income) |> 
  filter(!is.na(income_cat)) |> 
  group_by(campus_id, income_cat) |>
  summarise(
...
```{r}
varnames_original <- gsub("(.+?)(\\..*)", "\\1", names(css_means_income))
varnames_summarize <- names(css_means_income)
...
```{r}
css_means_income_long <- css_means_income |> 
  mutate(income_cat = factor(income_cat,
                             levels = c(0,1),
                             labels = c("income_cat_0", "income_cat_1"))) |> 
  pivot_longer(names_to = "variable", values_to = "mean",
               4:ncol(css_means_income)) |> 
  select(-n_responses) |> 
  pivot_wider(names_from = income_cat, values_from = mean)

css_means_income_merge <- left_join(css_means_income_long, income_cat_summary)
```


```{r}
css_means_income_wide <- css_means_income_merge |> 
  pivot_longer(names_to = "income_category", values_to = "mean",
               starts_with("income_cat")) |> 
  pivot_wider(names_from = "variable", values_from = "mean")

write.csv(css_means_income_wide, "../../data/css_means_income.csv", row.names = FALSE)
```

```{r}
css_means_income_merge_wtd <- css_means_income_wide |> 
  pivot_longer(names_to = "variable", values_to = "mean",
               5:ncol(css_means_income_wide)) |> 
  #select(-n_responses) |> 
  pivot_wider(names_from = income_category, values_from = mean) |> 
  group_by(variable) |> 
  summarise(wtd_income_cat_0 = wtd.mean(income_cat_0, n_responses*mean_low_income),
            wtd_income_cat_1 = wtd.mean(income_cat_1, n_responses*(1-mean_low_income)),
            wtd_diff = wtd_income_cat_0 - wtd_income_cat_1)
```


### Nothing below has been changed in income setup
