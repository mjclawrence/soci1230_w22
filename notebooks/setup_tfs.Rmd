...
tfs_means <- tfs_subset_master |> 
  ...
  summarise(across(starts_with("CHOICE"),
            ...
            across(starts_with("FUTACT"),
                   list(mean = ~mean(.x==4, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.verygoodchance_{.fn}"),
            across(starts_with("GOAL"),
                   list(mean = ~mean(.x>=3, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.veryimportant_essential_{.fn}"),
            across(starts_with("HSGPA"),
                   list(mean = ~mean(.x>=7, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.a_{.fn}"),
            across(starts_with("INCOME"),
                   list(mean = ~mean(.x>=11, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.75kplus_{.fn}"),
            across(starts_with("MAJORA"),
                   list(mean = ~mean(.x==3, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.business_{.fn}"),
            across(starts_with("PLANLIVE"),
                   list(mean = ~mean(.x %in% 3:5, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.oncampus_{.fn}"),
            across(starts_with("POLIVIEW"),
                   list(mean = ~mean(.x>=4, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.liberal_or_farleft_{.fn}"),
            across(starts_with("RACEGROUP"),
                   list(amindian_mean = ~mean(.x==1, na.rm = TRUE),
                        asian_mean = ~mean(.x==2, na.rm = TRUE),
                        black_mean = ~mean(.x==3, na.rm = TRUE),
                        hispanic_mean = ~mean(.x==4, na.rm = TRUE),
                        white_mean = ~mean(.x==5, na.rm = TRUE),
                        other_mean = ~mean(.x==6, na.rm = TRUE),
                        multiracial_mean = ~mean(.x==7, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}_{.fn}"),
            across(starts_with("RATE"),
                   list(mean = ~mean(.x>=4, na.rm = TRUE),
                        propna = ~sum(is.na(.)) / n()),
                   .names = "{.col}.aboveavg_exceptional_{.fn}"),
  ...
  summarise_at(vars(-n_responses), 
                   list(cor_mr_kq5_pq1 = ~wtd.cor(.x, mr_kq5_pq1, 
                        ...
                        cor_par_q5 = ~wtd.cor(.x, par_q5, 
                                              weight = n_responses)[1]),
                   .names = "{.col}_{.fn}") |>
  pivot_longer(names_to = "Variable", values_to = "Correlation", 1:1824) |>
...
tfs_correlations$Name <- gsub("(.+?)(\\..*)", "\\1", tfs_correlations$Name)

tfs_correlations <- left_join(varnames[,-3], tfs_correlations)
